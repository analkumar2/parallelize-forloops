...
def worker_append(func,L,i, seed, args):
...
def worker_save(func,i, seed, args,q):
    np.random.seed(seed+i)  #In case func is using any random number generation, this is needed so that the different processes dont generate the same random number
    out = func(args)  #Evaluate func
...
def Multiprocessthis_append(func, arguementlist, appendlists, seed=123):
    ...
    pool = mp.Pool(mp.cpu_count() + 2) #start the multiprocess pool
    
    #fire off workers
    jobs = []
    for i in range(len(arguementlist)):
        job = pool.apply_async(worker_append, (func,L,i,seed,arguementlist[i]))
        jobs.append(job)
...
def Multiprocessthis_save(func, arguementlist, filelists, seed=123):
    ...
    jobs = []
    for i in range(len(arguementlist)):
        job = pool.apply_async(worker_save, (func,i,seed,arguementlist[i],q))
        jobs.append(job)
